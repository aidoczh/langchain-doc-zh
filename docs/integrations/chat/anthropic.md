...
有关设置说明，请参阅[人类中心平台页面](/docs/integrations/platforms/anthropic.mdx)的安装和环境设置部分。
...
您可以在[这里](https://docs.anthropic.com/claude/docs/models-overview#model-comparison)查看模型比较文档。
...
使用人类中心的[工具调用或工具使用 API](https://docs.anthropic.com/claude/docs/functions-external-tools)，您可以定义模型调用的工具。这对于构建使用工具链和代理非常有用，也可以从模型获得结构化输出。
...
[BaseChatModel.with_structured_output 接口](/docs/how_to/structured_output) 使得从聊天模型获取结构化输出变得轻而易举。您可以使用 `ChatAnthropic.with_structured_output`（在底层使用工具调用）来让模型更可靠地返回特定格式的输出：
...
我们可以使用带有适当的 `tool_call_id` 的 `ToolMessage` 将工具结果传递回模型：
