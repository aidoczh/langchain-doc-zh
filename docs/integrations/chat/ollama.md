...
要查看支持的模型和模型变体的完整列表，请参阅[Ollama模型库](https://github.com/jmorganca/ollama#model-library)。

## 设置

首先，按照[这些说明](https://github.com/jmorganca/ollama)设置和运行本地的 Ollama 实例：

* [下载](https://ollama.ai/download)并安装 Ollama 到可用的支持平台上（包括 Windows Subsystem for Linux）
...
* 指定感兴趣的模型的确切版本，例如 `ollama pull vicuna:13b-v1.5-16k-q4_0`（在此示例中查看 `Vicuna` 模型的[各种标记](https://ollama.ai/library/vicuna/tags)）
...
您可以在[API 参考页面](https://api.python.langchain.com/en/latest/llms/langchain.llms.ollama.Ollama.html)上查看支持的参数的完整列表。

如果您使用的是 LLaMA `chat` 模型（例如 `ollama pull llama3`），那么您可以使用 `ChatOllama` 接口。

这包括[特殊标记](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)用于系统消息和用户输入。
...
查看所有端点的 Ollama [API 文档](https://github.com/jmorganca/ollama/blob/main/docs/api.md)。
...
查看[LangChain Expressive Language (LCEL) Interface](/docs/concepts#interface)以了解创建链时可用的其他接口。

## 从源代码构建

要获取最新的从源代码构建说明，请查看 Ollama 文档中的[从源代码构建](https://github.com/jmorganca/ollama?tab=readme-ov-file#building)。

## 提取

使用最新版本的 Ollama 并提供 [`format`](https://github.com/jmorganca/ollama/blob/main/docs/api.md#json-mode) 标志。`format` 标志将强制模型以 JSON 格式生成响应。

> **注意：** 您也可以尝试使用实验性的 [OllamaFunctions](/docs/integrations/chat/ollama_functions) 包装器来方便地进行尝试。
...
Ollama 支持多模态 LLM，例如 [bakllava](https://ollama.ai/library/bakllava) 和 [llava](https://ollama.ai/library/llava)。

通过 `tags` 浏览带有模型的完整版本，例如 [Llava](https://ollama.ai/library/llava/tags)。
