...
本文档展示了如何使用代理程序来比较两个文档。

高层次的思路是我们将为每个文档创建一个问答链，然后使用该链
...
这种类型的代理程序允许一次调用多个函数。当一些步骤可以并行计算时，比如要求比较多个文档时，这是非常有用的。
...
```output
{'input': '谷歌母公司或特斯拉哪个收入更高？',
 'output': '谷歌母公司的收入为697.87亿美元，而特斯拉的收入为233.29亿美元。因此，谷歌母公司的收入高于特斯拉。'}
```